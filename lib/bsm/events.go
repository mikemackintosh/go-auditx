...
func ParseEvents(eventsFile string) EventsDictionary {
	eventDefinitions, err := ParseEventDictionary(eventsFile)
	if err != nil {
		panic(err)
	}
...
func ParseEventDictionary(eventsFile string) (EventsDictionary, error) {
	var ed = EventsDictionary{}

	scanner := bufio.NewScanner(strings.NewReader(eventsFile))
